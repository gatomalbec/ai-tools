...
| **State files** | `.tdarlm/sessions/<session-id>/state/` | Arbitrary persistent intermediate data (e.g., `orient.md`, `dependency-graph.json`, `requirements.json`) |
...
| Requirements summary | Derived from `state/requirements.json` when present | Surfaces open/assumed/confirmed counts and critical blockers |
...
9 tools registered for the LLM:
...
| `rlm_requirements_status` | Summarize requirements closure and critical blockers from `requirements.json` |
...
### Requirements contract (recommended)

Track requirements closure in `.tdarlm/sessions/<session-id>/state/requirements.json`:
...
```json
{
  ...
  "questions": [
    { "id": "Q-001", "text": "clarification question", "status": "open" }
  ],
  "assumptions": [
    { "id": "A-001", "text": "temporary assumption", "status": "active" }
  ]
}
```

The observation includes a requirements summary (open/assumed/confirmed + blockers), and `rlm_requirements_status` exposes it as a tool.
...
- Phase 0.5: Contract Requirements (clarify and gate before decomposition)
